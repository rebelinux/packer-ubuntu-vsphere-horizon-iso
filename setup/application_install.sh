...
then 
    ...
    then
        ...
        then 
            ...
            DEBIAN_FRONTEND=noninteractive apt-get -qq -y install ./google-chrome-stable_current_amd64.deb 1> /dev/null
            retval=$?
            if [ $retval -ne 0 ]
            then 
...
then 
    ...
    DEBIAN_FRONTEND=noninteractive apt-get -qq -y install firefox 1> /dev/null
...
then 
    ...
    then
        ...
        then 
            ...
            DEBIAN_FRONTEND=noninteractive apt-get -qq -y install ./vscode.deb 1> /dev/null
            retval=$?
            if [ $retval -ne 0 ]; 
            then
